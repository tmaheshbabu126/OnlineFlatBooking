...
class IFlatServiceTest {
	...
	void testAddFlat() {
		...
		Mockito.when((Flat)this.farepo.save(flat1)).thenReturn(flat1);
	...
	void testUpdateFlat() {
		...
		Mockito.when(this.farepo.findById(1)).thenReturn(flat2);
		flat1.setFlatId(2);
		
		Mockito.when((Flat)this.farepo.save(flat1)).thenReturn(flat1);
		
		try {
			Assertions.assertThat(this.flatservice.updateFlat(flat1)).isEqualTo(flat1);
	...
	void testRempoveFlat() throws FlatNotFoundException {
		...
		Mockito.when(farepo.findById((int) 2)).thenReturn(flat2);
		Mockito.when(farepo.existsById(flat1.getFlatId())).thenReturn(false);
		assertFalse(this.farepo.existsById(flat1.getFlatId()));
	...
	void testViewFlat() throws FlatNotFoundException, Exception {
		...
		Optional<Flat> faList = Optional.of(flat1);
		 ...
		 Mockito.when(this.farepo.findById(1)).thenReturn(faList);
	        Assertions.assertThat(this.flatservice.viewFlat(1)).isEqualTo(faList);
	...
	void testViewAllFlat() {
        ...
        Mockito.when(this.farepo.findAll()).thenReturn(faList);
	...
	void testViewAllFlatByCost() {
		...
		flat1.setAvialibilty("Available");
		Mockito.when((Flat)this.farepo.save(flat1)).thenReturn(flat1);
		Assertions.assertThat(this.flatservice.viewAllFlatByCost(4500, "Available"));
	}
